...
EmbedReferenceOutputs ChatterboxConditionalsComponent::prepare_generation_reference(
    ...
    outputs.prompt_mel_ms = engine::debug::elapsed_ms(prompt_mel_start);
    ...
    outputs.speaker_ms = engine::debug::elapsed_ms(speaker_start);
    ...
    outputs.tokenizer_ms = engine::debug::elapsed_ms(gen_tokenizer_start);
    ...
    outputs.prompt_tokens = std::move(gen_prompt_tokens.tokens);
    outputs.prompt_token_count = gen_prompt_tokens.token_count;
...
TokenizerOutputs ChatterboxConditionalsComponent::tokenize_generation_audio(
    const runtime::AudioBuffer & audio,
    std::optional<int64_t> max_len) const {
    return tokenizer_.tokenize(audio, max_len);
}
...
ChatterboxConditionalsOutputs ChatterboxConditionalsComponent::prepare(
    ...
    outputs.gen = prepare_generation_reference(audio);
